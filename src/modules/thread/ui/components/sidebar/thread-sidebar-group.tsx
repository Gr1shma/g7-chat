...
import { SidebarHistoryProps } from "./thread-sidebar.types";
...
export function SidebarHistory({ user, searchQuery }: SidebarHistoryProps) {
    ...
    const [data, query] = api.thread.getInfiniteThreads.useSuspenseInfiniteQuery(
        { limit: 20 },
                            ...
                            return (
                                <div key={label}>
                                    <div className="px-2 py-1 text-xs text-sidebar-foreground/50">
                                        {labelMap[label as keyof typeof groupedThreads]}
                                    </div>
                                    {threads.map((thread) => (
                                        <ThreadItem
                                            key={thread.id}
